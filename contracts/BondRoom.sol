...
/// @title BondRoom — Escrow with deposit pattern (no approve needed)
/// Flow: user sends USDC to contract → calls deposit() → creates/funds rooms
contract BondRoom {
    ...
    // Each user has a "balance" in the contract (deposited USDC)
    // Part of it may be "locked" in active rooms
    ...
    // ═══ DEPOSIT: 2-step (send USDC to contract, then call deposit) ═══

    function deposit(uint256 _amount) external nonReentrant {
        require(_amount > 0);
        require(usdc.balanceOf(address(this)) >= _totalBal + _amount, "send USDC first");
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        require(balance[msg.sender] - locked[msg.sender] >= _amount, "locked");
        balance[msg.sender] -= _amount;
        _totalBal -= _amount;
    ...
    function available(address _u) external view returns (uint256) {
        return balance[_u] - locked[_u];
    }
        ...
        bytes32 _codeHash, bool _isSeller) external nonReentrant returns (uint256)
    {
        ...
        if (_collateral > 0) {
            require(balance[msg.sender] - locked[msg.sender] >= _collateral, "deposit collateral");
            locked[msg.sender] += _collateral;
        }
    ...
    function fundRoom(uint256 id) external nonReentrant onlyBuyer(id) inState(id, State.Joined) {
        ...
        require(balance[buyer] - locked[buyer] >= exact, "deposit funds");
        locked[buyer] += r.priceUSD; // lock price in escrow
        // Fee leaves contract
        balance[buyer] -= fee;
        _totalBal -= fee;
        require(usdc.transfer(treasury, fee));
    ...
    // ═══ SETTLEMENT ═══
    // Winner gets USDC from contract. Credit winner's balance, debit loser's.
    ...
    function releaseFunds(uint256 id) external nonReentrant onlyBuyer(id) inState(id, State.Delivered) {
        ...
        _transferOut(seller, price + collat);
        balance[seller] += price + collat; // credit seller
    ...
    function buyerRefund(uint256 id) external nonReentrant onlyBuyer(id) inState(id, State.Funded) {
        ...
        uint256 collat = r.collateralAmount;
        _closeRoom(r, seller, buyer);
        // Buyer gets price + collateral
        _transferOut(buyer, price + collat);
        balance[buyer] += price + collat;
    ...
    function arbiterResolve(uint256 id, address _w) external nonReentrant onlyArbiter inState(id, State.Disputed) {
        ...
        uint256 collat = r.collateralAmount;
        _closeRoom(r, seller, buyer);
        _transferOut(_w, price + collat);
        balance[_w] += price + collat;
    ...
    function arbiterSplit(uint256 id) external nonReentrant onlyArbiter inState(id, State.Disputed) {
        ...
        _transferOut(seller, half);
        _transferOut(buyer, total - half);
        balance[seller] += half;
        balance[buyer] += total - half;
        emit DisputeResolved(id, address(0), total);
    }

    function _closeRoom(BondRoom storage r, address seller, address buyer) internal {
        if (r.collateralAmount > 0) locked[seller] -= r.collateralAmount;
        if (r.fundedAmount > 0) locked[buyer] -= r.fundedAmount;
        r.fundedAmount = 0;
    ...
    function _transferOut(address _to, uint256 _amount) internal {
        if (_amount == 0) return;
        require(usdc.transfer(_to, _amount));
        _totalBal -= _amount;
    }
    ...
    function cancelRoom(uint256 id) external nonReentrant inState(id, State.Created) {
        ...
        if (r.collateralAmount > 0) {
            locked[msg.sender] -= r.collateralAmount;
            r.collateralAmount = 0;
        }
    ...
    function leaveRoom(uint256 id) external nonReentrant {
        ...
        if (r.collateralAmount > 0) {
            ...
            if (s == address(0)) s = r.creator;
            locked[s] -= r.collateralAmount;
            r.collateralAmount = 0;
        }
        emit RoomCancelled(id, msg.sender);
    }
