...
/// @title BondRoomV10 — Trustless Escrow with Seller Collateral
/// @notice Free create/join, 1% tax on fund, seller/buyer choice, collateral protection
/// @dev Arc Testnet (USDC native gas token)

contract BondRoomV10 {
    ...
    struct BondRoom {
        ...
        uint256 priceUSD;
        uint256 collateralAmount;   // seller must lock this much
        bool    collateralLocked;    // has seller locked collateral?
        uint256 collateralInContract; // NEW: tracks collateral separately from price
        uint32  createdAt;
        uint32  joinedAt;
        uint32  deliveredAt;
        uint32  disputedAt;
    ...
    event CollateralLocked(uint256 indexed roomId, uint256 amount);
    ...
    function lockCollateral(uint256 _roomId) external payable nonReentrant
        onlySeller(_roomId)
        inState(_roomId, State.Funded)
    {
        BondRoom storage room = rooms[_roomId];
        require(room.collateralAmount > 0, "No collateral required");
        require(!room.collateralLocked, "Collateral already locked");
        require(msg.value >= room.collateralAmount, "Insufficient collateral");

        room.valueInContract += room.collateralAmount;
        room.collateralInContract = room.collateralAmount;
        room.collateralLocked = true;

        uint256 excess = msg.value - room.collateralAmount;
        if (excess > 0) {
            (bool ok, ) = payable(msg.sender).call{value: excess}("");
            require(ok, "Excess refund failed");
        }

        emit CollateralLocked(_roomId, room.collateralAmount);
    }
    ...
    {
        BondRoom storage room = rooms[_roomId];
        // If collateral is required, it must be locked first
        require(room.collateralAmount == 0 || room.collateralLocked, "Lock collateral first");
        ...
        inState(_roomId, State.Delivered)
    {
        BondRoom storage room = rooms[_roomId];
        address seller = room.creatorIsSeller ? room.creator : room.counterparty;
        uint256 amount = room.valueInContract;  // price + collateral
        uint256 collateral = room.collateralInContract;
        ...
        emit RoomReleased(_roomId, room.priceUSD, collateral);
    }
        ...
        inState(_roomId, State.Funded)
    {
        ...
        uint256 collateral = room.collateralInContract;
        uint256 total = room.valueInContract;  // price + collateral
        room.valueInContract = 0;
        room.collateralInContract = 0;
        room.platformFee = 0;
        ...
        (bool ok, ) = buyer.call{value: total}("");
        require(ok, "Buyer refund failed");

        emit RoomRefunded(_roomId, price, collateral);
    }
        ...
        inState(_roomId, State.Disputed)
    {
        ...
        uint256 collateral = room.collateralInContract;
        uint256 price = room.valueInContract - collateral;
        room.valueInContract = 0;
        room.collateralInContract = 0;
        ...
        if (_winner == seller) {
            // Seller wins: gets price, buyer keeps collateral as consolation
            (bool ok, ) = seller.call{value: price}("");
            require(ok, "Arbiter payout failed");
            if (collateral > 0) {
                (bool ok2, ) = buyer.call{value: collateral}("");
                require(ok2, "Collateral payout failed");
            }
        } else {
            // Buyer wins: gets everything
            (bool ok, ) = buyer.call{value: price + collateral}("");
            require(ok, "Arbiter payout failed");
        }

        emit DisputeResolved(_roomId, _winner, price + collateral);
    }
        ...
        inState(_roomId, State.Disputed)
    {
        ...
        // Split price 50/50, collateral goes to buyer
        pendingWithdrawal[seller] += price / 2;
        pendingWithdrawal[buyer] += (price - price / 2) + collateral;

        emit DisputeResolved(_roomId, address(0), price + collateral);
    }
        ...
        inState(_roomId, State.Disputed)
    {
        ...
        // Seller ignored dispute → buyer gets everything back
        (bool ok, ) = buyer.call{value: price + collateral}("");
        require(ok, "Timeout refund failed");
    ...
    receive() external payable { revert("Use fundRoom or lockCollateral"); }
}
