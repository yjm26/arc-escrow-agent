...
async function main() {
  ...
  const BondRoomV11 = await hre.ethers.getContractFactory("BondRoomV11");
  const contract = await BondRoomV11.deploy(treasury, arbiterName);
  await contract.waitForDeployment();

  const address = await contract.getAddress();
  console.log("BondRoomV11 deployed to:", address);
